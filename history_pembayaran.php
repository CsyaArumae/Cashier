...
<?php 
    ...
    <div class="Container col-md-9 square rounded p-3" style="margin-left:auto; margin-right:auto; border-color:white; background-color:white; text-align:left;">
        <div class="card-header" style="color:white; background-color:white; text-align:left">
           ...
           <center><h3><font color="#9300a0">Payment History</font><br></h3></center>
            ...
            <Input class="form-control me-1" type="search" name="cari" placeholder="type id_menu here">
    ...
    <tbody style="color:white;">
        ...
        if (isset($_POST["cari"])){
            ...
            "SELECT * FROM pembayaran where nomor_meja = ' $cari ' or id_menu like '$cari%' ORDER BY nomor_meja DESC");
        }else {
        ...
        <?php
        $query_menu = mysqli_query($koneksi, "select * from menu where id_menu = '".$data_bayar['id_menu']."'");
        $data_menu = mysqli_fetch_array($query_menu);
        ?>
              <tr style="color:black">
              <input type="hidden" value="<?php echo $data_bayar['id_pembayaran']?>">
                  <td><?php echo $data_bayar["id_menu"]; ?></td>
                  <td><?php echo $data_bayar["nama_menu"]; ?></td>
                  <td><?php echo $data_menu["harga"]?></td>
                  <td><?php echo $data_bayar["qty"]; ?></td>
